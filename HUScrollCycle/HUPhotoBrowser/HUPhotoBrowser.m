...
+ (instancetype)showFromImageView:(UIImageView *)imageView withURLStrings:(NSArray *)URLStrings placeholderImage:(UIImage *)image atIndex:(NSInteger)index dismiss:(DismissBlock)block {
    ...
    if (URLStrings == nil || URLStrings.count == 0) {
        [browser addGestureRecognizer:[[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(dismiss)]];
    }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    if (self.URLStrings) {
        ...
        if (indexPath.row != _index) {
            ...
            UIImage *placeHolder = _tmpImageView.image ?: self.placeholderImage;
...
- (void)animateImageViewAtIndex:(NSInteger)index {
    ...
    tempImageView.image = self.imageView.image ?: self.placeholderImage;
